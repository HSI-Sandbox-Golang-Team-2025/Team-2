...
type UserPractice struct {
	...
	UserTrackID         uint                                       `json:"userTrackId" example:"1" extensuins:"x-order=07"`
	Status              UserPracticeStatus                         `json:"status" example:"opened" gorm:"type:user_practice_status" extensions:"x-order=08"`
	Score               float32                                    `json:"score" example:"100" extensions:"x-order=09"`
	Comment             *string                                    `json:"comment" example:"Good job!" extensions:"x-order=10"`
	UserPracticeRecords *[]user_practice_record.UserPracticeRecord `json:"userPracticeRecords" extensions:"x-order=11"`
}
